...
void BleKeyboard::begin(void)
{
  ...
  if (this->enable_passkey){
    pSecurity->setStaticPIN(this->passkey);
...
//must be called before begin in order to set passkey
void BleKeyboard::setPasskey(uint32_t passkey) {
  this->passkey = passkey; 
  this->enable_passkey = 1;	
}
/**
...
// USB HID works, the host acts like the key remains pressed until we
// call release(), releaseAll(), or otherwise clear the report and resend.
size_t BleKeyboard::press_iso(uint8_t k)
{
	uint8_t i;
...
// sends the report.  This tells the OS the key is no longer pressed and that
// it shouldn't be repeated any more.
size_t BleKeyboard::release_iso(uint8_t k)
{
	uint8_t i;
	k = pgm_read_byte(_asciimap_iso + k);
	if (!k) {
...
size_t BleKeyboard::write_iso(uint8_t c)
{
	String cn;
...
size_t BleKeyboard::write_iso(const uint8_t *buffer, size_t size) {
	size_t n = 0;
	while (size--) {
		if (*buffer != '\r') {
			if (write_iso(*buffer)) {
...
byte BleKeyboard::utf8ascii(byte ascii) {
...
String BleKeyboard::utf8ascii(String s)
{       
	String r="";
	char c;
...
void BleKeyboard::utf8ascii(char* s)
{       
	int k=0;
	char c;
